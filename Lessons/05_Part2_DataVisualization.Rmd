...
O3plot3 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_light()
print(O3plot3)#grey lines in the axis 
...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
mytheme <- theme_classic(base_size = 14) + #increase fonot size 
  theme(axis.text = element_text(color = "black"), #change text axis text to grey 
        legend.position = "top") #alternative: legend.position + legend.justification
#base size is 11 in defaults 
...
print(O3plot7)  
#line that crosses the intercept 
#lty is the type of line 
#points representing data set alpha will determine transparency and size of the point  hjust = 1 means left align & adding text to the plot and add where you want it to be added
#defining three different levels 
#in relation to the line the points are at a certain level 
...
#compbined both plots together rel_heighter adjusted the space for the plots because one might need more space than the other 
#you need to use this to show it in the matrix 
```
