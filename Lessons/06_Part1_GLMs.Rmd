...
# rsquare is telling you how much of the variability the x nd y series is trying to fit, a low rsquared means that the line only explains a small amount of the values. The P value- is telling you the significance of the equations - if the corefficent you found is not different from zero 
this will tell you if the linear regression is good or not
...
#variable y to one single variable x 
#lm means linear model ( what is Y ~ and our X indepent variable)
...
#par(mfrow = c(1,1))
#dividing the cell into 4 cells into a 2x2 gird  
#
```
...
#we see how we can describe the relationship with a straight line 
...
#good way to spot important values to include 
```
...
#helps you undestand the variables to add to the comparison. Used for model selection
#if AIC goes up don't remove it, keep the variables to get the lowest AIC don't want variables with high correlation because they will be trying to explain the same thing
...
step(TPAIC) TPmodel <- lm(data = Paul.naomit, tp_ug ~ dissolvedOxygen + temperature_C + tn_ug)
summary(TPmodel)
