...
#you have to know how the data is stored so you knwo what to add to yout code 

# dplyr filtering
NTL.phys.data.surface <- filter(NTL.phys.data, depth == 0)

# Choose multiple conditions to filter
summary(NTL.phys.data$lakename) #this will show you all the lakes 
NTL.phys.data.PeterPaul <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) #c creates vectors with more than one element 
...
NTL.phys.data.Tuesday.Lake <- filter (NTL.phys.data, lakename == "Tuesday Lake")
NTL.phys.data.Tuesday.1990to1999 <- filter(NTL.phys.data.Tuesday.Lake, year4 %in% c(1990:1999))

#come back to this the code didn't work 
```
...
NTL.phys.data.gathered <- 
  pivot_longer(NTL.phys.data, irradianceWater: irradianceDekc, names_to = "irradiance", values_to = "radiation")
