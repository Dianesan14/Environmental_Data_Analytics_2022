...
#first line you add data frame and the second layer you add aes.
#datafram should always be in the first spot, when you want to add layers to ggplo you use the +, the plus is the operator. 

# Fix this code
O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone), color = "blue")
print(O3plot2)
 #geom_point(aes(x = Date, y = Ozone, color = "blue")) make sure you add the layer to the plot on the outside of the 
#adding layers if you add into the aes. make sure do add color outside 

# Add additional variables
# How could you automatically assign a marker color to a variable? define a different shape to differnt years. If 
PMplot <- 
  ggplot(EPAair, aes(x = Month, y = PM2.5, shape = as.factor(Year), color = Site.Name)) +
  geom_point()
print(PMplot)

#does show much about the year or site name 
#shape based on teh year - these are assigning differenct shape based on year 
...
#creating one plot for each of the sites. you can see teh separate sites and still see the circle and triangles and are still separted by month bc we wanted to see how they change throughout the year. Crate one plot for each site 
...
#you can always plot a subset of the data frame
#inside ggplot oyu can select just the sites you want to plot 
...
PMplot.line <- 
  ...
  geom_line()
print(PMplot.line)
#connect points so you have to switch axis. this is a typical picture of a time series plot 
...
lightvsDO <- 
  ...
  geom_point()
print(lightvsDO)
#relationship with irradiance water and dissolved oxygen. you are still doing a scatter plot with two separate variables and finding the relationship. 
...
#adjuct the window by repeating commands but now change the xlimit and y limit 
...
tempvsdepth <- 
  ...
  scale_y_reverse()
print(tempvsdepth)
# you can imagine this plot as an ocean 

NvsP <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth)) +
  geom_point() +
  geom_smooth(method = lm) + #trend line 
  geom_abline(aes(slope = 16, intercept = 0)) #add slope and intercept 
print(NvsP)
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? regards the contor of the shape 
print(Nutrientplot3)

# Dot plot
Nutrientplot4 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
  geom_dotplot(aes(color = nutrient, fill = nutrient), binaxis = "y", binwidth = 1, #you are filling the inside of the dots with colots and the category is the nutrient. 
...
print(Nutrientplot6)
#varialble you want on the x axis and the color you only in the same line 
# Using a gathered dataset. orginally doens't have legend and only has tn_ug 

Nutrientplot7 <-   
  ggplot(PeterPaul.chem.nutrients.gathered) +
  geom_freqpoly(aes(x = concentration, color = nutrient))
print(Nutrientplot7)
#gathered data set #color inside the aesthetic you will get the legend 
...
print(Nutrientplot6)
#categorizing y by nurtients and one density and ridge plot of each lake used fill bc she wanted to color inside of lines 
