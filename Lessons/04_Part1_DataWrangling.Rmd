...
#when filtering you can only compare same classes 

# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,]
#filtering rows that have 0 in the depth column 
...
#peter paul 2 is excluding the lakes you don't want- this is exculting those names 
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake"))
#this will be useful when you have to use more than two elements 


# Choose a range of conditions of a numeric or integer variable
summary(NTL.phys.data$daynum) #opening data set and checking the number where observations start, you look at the data and select the values you want to see 
NTL.phys.data.JunethruOctober1 <- filter(NTL.phys.data, daynum > 151 & daynum < 305)#with > value 151 will not be included and < 305 becasue it will not select 205 
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305) #the comma means combination of both 
NTL.phys.data.JunethruOctober3 <- filter(NTL.phys.data, daynum >= 152 & daynum <= 304)#if you want to use the actual number you have to use >= or <= 
NTL.phys.data.JunethruOctober4 <- filter(NTL.phys.data, daynum %in% c(152:304))#with %n% a value matching operating whatever is in the vector is what is going to be included in the code 
...
#mutate then add new column name = existing data in the OG coulnm 
...
interval(first(NTL.phys.data.PeterPaul1$sampledate), last(NTL.phys.data.PeterPaul1$sampledate))#this is better for any size of datasets 
```
...
we don't want to do everything in the same document bc the rmd file will be really long and having diffent files makes it easier to navigate the data 
