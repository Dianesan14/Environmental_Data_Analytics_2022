...
O3.onesample #within the threashold for good standard 
...
#used fill instead of color bc i wanted transparency and density to be filled with color
#fill with empty code bc anything i write will be ledge so you are making sure the legend has no name 
...
print(Box_plot)
#fun.y is applying function more than once, mark text on max number of the data
...
#this shows there are more things that are impacing ozone 
```
...
Ozonebyall_regression <- lm(data = EPAair_subset, PM2.5 ~ Ozone + Year + Month + meanLng + meanLat)
...
Ozonebest_regression <- lm(data = EPAair_subset, PM2.5 ~ Ozone + Month + meanLng + meanLat )

summary(Ozonebest_regression)
